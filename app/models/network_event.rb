  ...
  def photos_size_limit
    ...
    photos.each do |photo|
      if photo.byte_size > 2.megabytes
        ...
        errors.add(:photos, "contient une photo (#{photo.filename}) qui dépasse 2 Mo (taille actuelle : #{size_in_mb} Mo)")
      end
    end
  end
  ...
  def photos_format
    ...
    photos.each do |photo|
      unless photo.content_type.in?(allowed_types)
        errors.add(:photos, "contient un fichier (#{photo.filename}) au format non autorisé. Formats acceptés : JPEG, PNG, WebP")
      end
    end
  end
end
